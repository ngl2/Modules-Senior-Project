...
class Sale_lib
{
	...
	function add_item($item_id,$quantity=1,$discount=0,$price=null,$tax=null)
	{
		...
		{
			$items[$item_id]['quantity']+=$quantity; # then ask if we want to add to existing item's quantity.
		}
		else
		{
			# add item to table
	...
	function get_taxes()
	{
		...
		# check to ensure customer is indeed taxable
